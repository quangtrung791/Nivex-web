...
export async function GET(request, { params }) {
  try {
    ...
    const result = await query(
      `SELECT id, title, content, thumbnail_url, time_event, created_at, updated_at
  ...
  try {
    ...
    const updateSQL = `
      UPDATE public.event SET
        title=$1, content=$2, thumbnail_url=$3, time_event=$4, updated_at=NOW()
      WHERE id=$5
      RETURNING id, title, content, thumbnail_url, time_event, created_at, updated_at
    `
